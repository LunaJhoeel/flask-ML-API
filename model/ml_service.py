...
def predict(image_name):
    """
    Loads an image from the corresponding folder based on the image name
...
def classify_process():
    ...
    while True:
        ...
        _, dict_img = db.brpop(settings.REDIS_QUEUE)
        dict = json.loads(dict_img.decode('utf-8'))
        #   2. Run your ML model on the given data
        clas_name, probab = predict(dict["image_name"])
        #   3. Store model prediction in a dict with the following shape:
        pred_dict ={
                 "prediction": clas_name,
                 "score": probab,
              }
